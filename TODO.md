...
- [x] PROMPT 3 (structure) / [ ] (personnes) — Circulaire de nominations → `service_diocesain`/`commission_diocesaine`/`mouvement`/`association` : `inc/import/data-nominations.php` peuplé des 33 entités réelles (10/10/6/7, noms déjà documentés dans `SPEC.md` §3) + 4 sous-structures de l'Économat ; `responsable`/`membres`/`aumônier` réels non transcrits, **`NOMINATIONS_SERVICES_COMMISSIONS_AUMONERIES_2027.pdf` absent de ce dépôt** (voir `BUGS_AND_ROADMAP.md`). Les 4 fonctions d'import par CPT (`dz_import_run_nominations_*`) entièrement implémentées et testées (33 créations, idempotence sur second passage). `bin/seed-cpt-organisation.php` recentré sur "Conseil épiscopal" uniquement pour éviter un doublon avec ce nouvel import (voir `DECISIONS.md`)
